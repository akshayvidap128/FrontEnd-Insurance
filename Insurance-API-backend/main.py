...
# POC database
AURADB_URI = "neo4j+s://14b8d10c.databases.neo4j.io"
AURADB_USERNAME = "neo4j"
AURADB_PASSWORD = "xYCmdQ64S6FnUlP7345DDn-PJVKOEnpd03diqumuKXM"

# AURADB_URI = "neo4j://20.228.147.220:7687"
# AURADB_USERNAME = "neo4j"
# AURADB_PASSWORD = "Password@12345"
...
class Question:
    ...
    def marshal(cls, question, answer=None) -> "Question":
        ...
        return cls(
            ...
            tableUuid=question['table_uuid'],
            rowCount=question['row_count'],
            columnCount=question['column_count'],
            columnData=question['column_data'],
            rowData=question['row_data'],
            answer=answer
        )
...
class Application:
    ...
    @classmethod
    def marshal(cls, application, sections=[]) -> "Application":
...
class Query:
    ...
    def applications(self) -> List[Application]:
        with graph.session() as session:
            query = """
                ...
                RETURN a, COLLECT (distinct s) as s
            """
    ...
    @strawberry.field()
    def getTableQuestions(self, tableUuid: str) -> List[Question]:
        with graph.session() as session:
            query = f"""
                MATCH (q:Question)
                WHERE q.table_uuid = '{tableUuid}'
                RETURN q
            """
            rows = session.run(query).data()
        return [
    ...
    def getUserQuestions(
        self, applicationUuid: Optional[str] = None
            ) -> List[Question]:
        with graph.session() as session:
            query = f"""
                MATCH (a:Application) WHERE a.uuid='{applicationUuid}'
                MATCH (a)--(s:Section)--(q:Question)
                WHERE q.role = "User"
                RETURN q
            """
            rows = session.run(query).data()
